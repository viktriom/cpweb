...
public class NetworkClient {
	...
	public void connect() {
		if (null == serverIPAddr) {
			log.info("The server address provided is incomplete : serverIpAddr = " + serverIPAddr + "and port = "
					+ String.valueOf(port));
			return;
		}
		...
		try {
			...
			log.error(
					"There was an error while connecting to the server : " + serverIPAddr + ":" + String.valueOf(port));
			client = null;
			out = null;
			in = null;
			e.printStackTrace();
	...
	public String getResponseFromServer() {
		...
		try {
			...
			log.info("Exception encountered while reading data from the server : " + serverIPAddr + ":"
					+ String.valueOf(port));
			e.printStackTrace();
		}
	...
	public void sendMessageToServer(String message) {
		...
		try {
			...
			log.info("Exception encountered while sending message to the server : " + serverIPAddr + ":"
					+ String.valueOf(port));
			e.printStackTrace();
		}
	}
	...
	public void disConnect() throws IOException {
		if (null != client) {
			try {
				...
				log.info("Exception encountered while closing the connection to the server : " + serverIPAddr + ":"
						+ String.valueOf(port));
				e.printStackTrace();
			}
		}
